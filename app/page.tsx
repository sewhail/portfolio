...
export default function Page() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${isDark ? 'dark gradient-bg-dark' : 'gradient-bg-light'}`}>
      {/* Header */}
      <header className={`border-b glass-header sticky top-0 z-50 transition-colors duration-300 ${isDark ? 'border-white/5 bg-black/40' : 'border-black/5 bg-white/60'}`}>
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-3">
          <nav className="flex items-center justify-between">
            ...
            <Button
              ...
              size="sm"
              onClick={toggleTheme}
              className={`h-7 w-7 p-0 rounded-md transition-all duration-300 ${isDark
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <section id="about" className="mb-10 sm:mb-12">
          <div className={`flex flex-col sm:flex-row items-center sm:items-start gap-5 sm:gap-6 mb-6 transition-all duration-700 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <div className="text-center sm:text-left pt-0 sm:pt-3">
              <h1 className={`text-xl sm:text-2xl font-normal tracking-tight mb-1 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
                suhail
              </h1>
              <p className={`text-xs sm:text-sm font-light ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
                software engineer
              </p>
            </div>
          </div>

          <p className={`text-xs sm:text-sm leading-relaxed mb-6 max-w-lg font-light transition-all duration-700 delay-100 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          ...
          <div className={`flex flex-wrap gap-2 mb-6 transition-all duration-700 delay-200 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            {['fullstack', 'ml/ai', 'oss'].map((tag) => (
              <Badge
                key={tag}
                variant="outline"
                className={`text-[10px] px-2 py-0.5 font-light transition-all duration-300 skill-badge ${isDark
          ...
          <div className={`flex items-center gap-2 transition-all duration-700 delay-300 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <Button
              variant="ghost"
              size="sm"
              className={`h-7 text-[10px] sm:text-[11px] px-3 font-light transition-all duration-300 social-icon ${isDark
        ...
        >
          <div className={`flex items-center gap-2 mb-4 transition-all duration-700 ${currentSection.isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            ...
            <h2 className={`text-xs sm:text-sm font-light tracking-wide ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>
              /current
            </h2>
          </div>
          ...
          <div className="space-y-2">
            {[
              ...
              <div
                key={index}
                className={`flex items-center gap-3 py-2 px-3 transition-all duration-500 group cursor-default ${currentSection.isInView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
                  } ${isDark ? 'hover:bg-white/[0.02]' : 'hover:bg-black/[0.02]'}`}
                style={{ transitionDelay: currentSection.isInView ? `${index * 100 + 200}ms` : '0ms' }}
              >
                <span className={`text-[10px] transition-colors duration-300 ${isDark
                  ? 'text-gray-500 group-hover:text-emerald-400'
                  : 'text-gray-400 group-hover:text-emerald-600'
                  }`}>
                  &gt;
                </span>
                <span className={`text-xs font-light transition-colors duration-300 ${isDark
        ...
        >
          <h2 className={`text-xs sm:text-sm font-light tracking-wide mb-4 transition-all duration-700 ${stackSection.isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          ...
          <div className="flex flex-wrap gap-2">
            {['js', 'py', 'react', 'node', 'ts', 'psql', 'docker', 'aws'].map((skill, index) => (
              <span
                key={skill}
                className={`px-3 py-1.5 text-[10px] sm:text-[11px] font-light border transition-all duration-500 cursor-default ${stackSection.isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                  } ${isDark
        ...
        >
          <h2 className={`text-xs sm:text-sm font-light tracking-wide mb-4 transition-all duration-700 ${contactSection.isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            } ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>
            /contact
          </h2>
          ...
          >
            <CardContent className="p-5 sm:p-6">
              <p className={`text-xs sm:text-sm mb-5 font-light leading-relaxed ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <a
                  ...
                  className={`inline-flex items-center gap-2 text-xs font-light transition-all duration-300 animated-underline ${isDark
